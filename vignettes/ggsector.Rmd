...
The default is TRUE, mainly to control whether to draw sector by sector with a single coordinate point or to draw as a whole on the drawing board in the form of vector. <br>
When individual = TRUE, draw one by one, the sector will not be deformed, but when there are too many sectors drawn, the overall drawing speed will be much slower. <br>
When individual = FALSE, drawing in vector form is faster, but it needs to be used with coord_fixed() to lock the aspect ratio of the drawing board, otherwise the sector will be deformed.
