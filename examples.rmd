...
ggsector是本人的第二个R包了，但是却是本人第一个面向公众的R包，
本包主要作用为在ggplot2、grid中可以轻易的绘制扇形。
在写这个包的过程中，笔者最先参考了J叔“老俊俊”的jjpie，随后参考了Hadley大佬的ggplot2代码，
在本R包接近完成之际，笔者参考了Y叔“余光创”老师的ggfun、treeio等包中DESCRIPTION、 vignette、readme的写法。
总之，本R包也是站在巨人的肩膀上才有的成果，感谢以上大佬。

## R包安装

目前本包暂时只能通过github来下载，地址为https://github.com/yanpd01/ggsector，
下载命令为`remotes::install_github("yanpd01/ggsector")`，
网不好的读者也可以通过gitee安装`remotes::install_git("https://gitee.com/yanpd01/ggsector")`

小小吐槽一下，CRAN审核好严格啊，从标题到描述、还有各种案列的使用，都审核的好仔细，
...
在ggplot2中使用ggsector非常简单，只需要使用geom_sector即可，美学映射
aes()中去除常规的x、y、fill、color外，还有其他几个可以高度自定义的参数。
...
### individual
默认为TRUE，主要控制是以向量的形式在画板上整体出图，还是以单个坐标点进行逐个扇形的绘制，
...
观察这个图里圈起来的地方（这里展示的可能不是很清楚，但读者可自行运行尝试），
便能发现，两种模式下的扇形并没有完全的重合在一起，这是因为
type = "percent"时，圆周为100个散点，输入的数值会被round()为0-100的整数，
type = "degree"时，圆周为360个散点，输入的数值会被round()为0-360的整数，
圆周点数越多，精度越高，但需要注意的是，越多的散点也意味着越慢的绘图速度，
笔者这里暂时只提供这两种模式，
后续会提供自定义圆周散点数目的功能，
敬请期待！
...
本R包并不是只能在ggplot中调用，本包的底层从grid写起，也为直接调用grid画扇形提供了方便

### sectorGrob

参数与ggplot2中，一模一样，笔者可以自行尝试
...
有三种方式，最终呈现结果基本一致，此处只配一幅图。
（不一致的地方是因为本案例中使用了runif()随机数，
而ComplexHeatmap中cell_fun与layer_fun调用方式也不一样，所以呈现出前两幅图一模一样，第三幅不一样）
...
笔者之前只想看看某个基因在某个细胞type中的表达情况（百分比与表达值），
Seurat自带的DotPlot能满足我一些需求，但又不是很直观，恰逢笔者写出来了ggsector，便正好对接一下。
以下以pmnc3K数据为例。

这里SectorPlot()函数，笔者就不仔细介绍了，感兴趣的小伙伴可以自行研究。
